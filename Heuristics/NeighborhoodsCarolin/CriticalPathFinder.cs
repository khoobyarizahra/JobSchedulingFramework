...
namespace JobShopSchedulingFramework.Heuristics.Neighborhoods
{
    public class CriticalPathFinder 
    {
        ...
        {
            // Speichert die letzte Operation im Schedule (Makespan Ende)
            Operation lastOp = null;

            int cmax = ScheduleEvaluator.CalculateCmax(jobs);
            ...
            while (current != null) // solange eine vorgänger operation existiert
            {
                ...
                current = FindPredecessor(current, jobs, state); // sucht die vorherige operation
            }
